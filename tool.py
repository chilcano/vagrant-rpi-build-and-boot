...
def netboot(filename, rdisk, ip):
  ...
  if yesno.find("y") != -1:
    ...
    with open(os.path.join(mount, "cmdline.txt"), "w") as f:
      f.write("dwc_otg.lpm_enable=0 console=ttyAMA0,115200 kgdboc=ttyAMA0,115200 console=tty1 elevator=deadline root=/dev/nfs rootfstype=nfs nfsroot=10.0.0.1:/opt/raspberrypi/root,udp,vers=3 rw fsck.repair=no rootwait ip=%s:10.0.0.1:10.0.0.1:255.255.255.0:rpi:eth0:off:8.8.4.4:8.8.8.8 smsc95xx.turbo_mode=N" % ip)
